...
import PoolReportModal from "./PoolReportModal";
...
const PoolDetails = () => {
  ...
  // Handler to open modal with selected report
  const handleReportClick = (report) => {
    // Pass everything to modal and ensure property names match modal expectation
    setSelectedReport({
      ...report,
  ...
  const fetchPoolDetails = async (isAutoRefresh = false) => {
    ...
    try {
      ...
      const [response] = await Promise.all([
        ADMIN_API.GET_LIVE_POOLS_NOT_RWA_DATA({
      ...
      console.log("GET_LIVE_POOLS_DATA Response", response);
  ...
  const fetchDailyRWAPoolsReports = async (isAutoRefresh = false) => {
    ...
    try {
      ...
      if (response?.data) {
        ...
        if (startDate && endDate && responseData.summary && (responseData.daily?.data || responseData.dailyReports)) {
          // Date range report structure
          const summary = responseData.summary[0];
          const dailyReports = responseData.daily?.data || responseData.dailyReports[0]?.dailyReports || [];
          ...
          setReportsTotalPages(responseData.daily?.totalPages || Math.max(1, Math.ceil(apiCount / reportsLimit)));
        } else {
          // Standard report structure
          const dataToProcess = responseData.data || responseData;
          const reportsData = Array.isArray(dataToProcess) ? dataToProcess : (dataToProcess.data || []);
  ...
  const handleAutoRefresh = async () => {
    ...
    try {
      ...
      const poolResponse = await ADMIN_API.GET_LIVE_POOLS_NOT_RWA_DATA({
        tokenAddress: tokenAddress,
        pairAddress: pairAddress,
        chainId: chainId,
      });

      console.log("GET_LIVE_POOLS_NOT_RWA_DATA Response", poolResponse);
      ...
      const reportsFetchPromise = (startDate && endDate)
        ? ADMIN_API.GET_RWA_POOL_DATE_RANGE_REPORT({
            startTime: numericStartTime,
            endTime: numericEndTime,
            pairAddress: pairAddress,
      ...
      if (reportsResponse?.data) {
        ...
        if (startDate && endDate && responseData.summary && (responseData.daily?.data || responseData.dailyReports)) {
          ...
          setReportsTotalCount(apiCount);
          setReportsTotalPages(responseData.daily?.totalPages || Math.max(1, Math.ceil(apiCount / reportsLimit)));
          // Keep pagination state in sync with API when date-range is used
          setReportsCurrentPage(responseData.daily?.currentPage || responseData.daily?.page || 1);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [chainId, pairAddress, tokenAddress, reportsCurrentPage, reportsLimit, startDate, endDate]);
  ...
  const formatCompactNumber = (num, isCurrency = false) => {
    if (num === null || num === undefined || num === 0) return isCurrency ? "$0" : "0";
  ...
  const formatVolume = (volume) => {
    if (typeof volume !== "number" || isNaN(volume)) return "$0.00";
  ...
  const exportToCSV = () => {
    ...
    const csvData = rwaReports.map((report) => [
      formatDate(report.startTime || report.endTime || report.date || report.day || report.timestamp),
      typeof report.totalVolume === "number"
        ? report.totalVolume.toFixed(2)
        : "0.00",
      typeof report.poolFee === "number"
        ? report.poolFee.toFixed(2)
        : "0.00",
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:gap-4 mb-3">
                    <h1 className="text-2xl sm:text-3xl lg:text-4xl font-black text-gray-900 mb-2 sm:mb-0">
                      {formatSymbol(poolData.mintA.symbol)}-
                      {formatSymbol(poolData.mintB.symbol)}
                    </h1>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (endDate) {
                              const range = { 
                                startTime: parseInt(newStartDate.replace(/-/g, "")), 
                                endTime: parseInt(endDate.replace(/-/g, "")) 
                              };
                              console.log("📊 API Date Range Filter (Numeric):", range);
                            }
                          }}
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (startDate) {
                              const range = { 
                                startTime: parseInt(startDate.replace(/-/g, "")), 
                ...
                <div className="mb-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 animate-in fade-in slide-in-from-top-4 duration-500">
                  <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-3 shadow-lg text-white">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <h4 className="text-xs font-semibold uppercase tracking-wider opacity-80">Total Volume</h4>
                    </div>
                    <p className="text-xl font-bold">{formatCompactNumber(totalSummary.totalVolume, true)}</p>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-blue-600 to-cyan-600 rounded-2xl p-3 shadow-lg text-white">
                    <div className="flex items-center gap-3 mb-2">
                      <DollarSign size={18} className="opacity-80" />
                      <h4 className="text-xs font-semibold uppercase tracking-wider opacity-80">Pool Fee</h4>
                    </div>
                    <p className="text-xl font-bold">{formatCompactNumber(totalSummary.poolFee, true)}</p>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-3 shadow-lg text-white">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <h4 className="text-xs font-semibold uppercase tracking-wider opacity-80">Transactions</h4>
                    </div>
                    <p className="text-xl font-bold">{formatCompactNumber(totalSummary.totalTransactions)}</p>
                  </div>
                  ...
                  <div className="bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-2xl p-3 shadow-lg text-white">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <h4 className="text-xs font-semibold uppercase tracking-wider opacity-80">Total Buys</h4>
                    </div>
                    <p className="text-xl font-bold">{formatCompactNumber(totalSummary.buys)}</p>
                  ...
                  <div className="bg-gradient-to-br from-red-500 to-red-600 rounded-2xl p-3 shadow-lg text-white">
                    <div className="flex items-center gap-3 mb-2">
                      <TrendingUp size={18} className="opacity-80 transform rotate-180" />
                      <h4 className="text-xs font-semibold uppercase tracking-wider opacity-80">Total Sells</h4>
                    </div>
                    <p className="text-xl font-bold">{formatCompactNumber(totalSummary.sells)}</p>
                    {/* <div className="flex items-center gap-2 mt-1">
                      <div className="h-1 flex-1 bg-white/20 rounded-full overflow-hidden">
                        <div 
                          className="h-full bg-white" 
              ...
              {reportsLoading ? (
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-500">
                    There are no pool reports for this pool yet.
                  </p>
                </div>
              ) : (
                        ...
                        >
                          <td className="py-4 px-4 text-sm font-medium text-gray-900 whitespace-nowrap text-center">
                            <div className="flex items-center gap-2 justify-center">
                              <span>{formatDate(report.startTime || report.endTime || report.date || report.day || report.timestamp)}</span>
                              {user?.role !== "superuser" && (
                                <button
                          ...
                          <td className="py-4 px-4 text-sm font-semibold text-green-600 whitespace-nowrap text-center">
                          ...
                          <td className="py-4 px-4 text-sm font-medium text-blue-600 whitespace-nowrap text-center">
                            {report.totalTransactions?.toLocaleString("en-US")}
                          </td>
                          <td className="py-4 px-4 text-sm font-medium text-orange-600 whitespace-nowrap text-center">
                          ...
                          {/* <td className="py-4 px-4 text-sm font-medium text-blue-600 whitespace-nowrap text-center">
                            {formatVolume(report.poolRevenue ?? 0)}
                          </td>
                          <td className="py-4 px-4 text-sm font-medium text-purple-600 whitespace-nowrap text-center">
                            {formatVolume(report.companysRevenue ?? 0)}
                          </td>
                          <td className="py-4 px-4 text-sm font-medium text-orange-600 whitespace-nowrap text-center">
                            {formatVolume(report.usersRevenue ?? 0)}
                          </td> */}
                ...
                <div className="bg-gradient-to-br from-blue-50 to-cyan-50 rounded-2xl p-5 border border-blue-200/50">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className={`w-10 h-10 bg-gradient-to-br from-blue-500 to-cyan-500 rounded-xl ${poolData.mintA.logoURI ? "hidden" : "flex"} items-center justify-center text-white font-bold`}>
                        {poolData.mintA.symbol.charAt(0)}
                      </div>
                      <div>
                ...
                <div className="bg-gradient-to-br from-gray-50 to-blue-50 rounded-2xl p-5 border border-gray-200/50">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className={`w-10 h-10 bg-gradient-to-br from-gray-600 to-blue-600 rounded-xl ${poolData.mintB.logoURI ? "hidden" : "flex"} items-center justify-center text-white font-bold`}>
                        {poolData.mintB.symbol.charAt(0)}
                      </div>
                      <div>
                        <h4 className="font-bold text-gray-900">
