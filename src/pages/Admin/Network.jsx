...
export default function Network() {
  ...
  const handleDownloadExcel = async () => {
    try {
      ...
      const firstResponse = await ADMIN_API.ADMIN_GET_NETWORKS({ page: 1, limit: 1 });
      const totalCount = firstResponse?.data?.count || firstResponse?.data?.totalRecords || 0;
      // Now fetch all data using the total count
      const response = await ADMIN_API.ADMIN_GET_NETWORKS({ page: 1, limit: totalCount });
      const networkData = response?.data?.data || [];
      const XLSX = await import('xlsx');
      const data = networkData.map((n, idx) => ({
        SNo: idx + 1,
        Name: n.name,
  ...
  const fetchNetworks = async (page = 1, limit = pagination.limit) => {
    try {
      setLoading(true);
      setError(null);
      const response = await ADMIN_API.ADMIN_GET_NETWORKS({ page, limit });
      if (response.data) {
        const {
          data: networkData = [],
          count,
        ...
        const normalizedNetworks = networkData.map((network) => ({
          ...network,
          status:
            network.status === true ||
            network.status === "true",
  ...
  const stats = [
    ...
    },
    {
      ...
      value: networks.filter((n) => n.status === true).length,
      icon: Zap,
      color: "green",
      accent: "from-emerald-500 to-green-600",
    },
    {
      title: "Inactive Networks",
      value: networks.filter((n) => n.status === false).length,
      icon: Activity,
      color: "purple",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingNetwork) {
        console.log('Updating network with data:', payload);
        const response = await ADMIN_API.ADMIN_UPDATE_NETWORKS(
          editingNetwork._id,
          payload
        );
        ...
        if (response.data) {
          setSuccess(
            response.data.message || "Network updated successfully!"
          );
          await fetchNetworks(pagination.currentPage);
        }
      } else {
        console.log('Creating network with data:', payload);
        const response = await ADMIN_API.ADMIN_CREATE_NETWORKS(payload);
        console.log('Create response:', response.data);
        if (response.data) {
          setSuccess(
            response.data.message || "Network created successfully!"
          );
          await fetchNetworks(pagination.currentPage);
        }
      }
      ...
      setError(
        ...
        `Failed to ${editingNetwork ? "update" : "create"} network`
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleDelete = (id) => {
    // Note: Delete endpoint not provided in the API
    alert("Delete functionality is not available yet. Please contact the administrator.");
  };
                ...
                <tbody className="divide-y divide-gray-200">
                  {networks.length === 0 ? (
                    <tr>
                      ...
                      >
                        No networks found. Click "Add Network" to create one.
                      </td>
                    </tr>
                  ) : (
                    networks.map((network, index) => (
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-3 py-1 rounded-full text-sm font-semibold ${network.status
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            {/* <button
                              ...
                              className="p-2 bg-red-100 hover:bg-red-200 text-red-600 rounded-lg transition-colors duration-200 opacity-50 cursor-not-allowed"
