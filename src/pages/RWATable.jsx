...
//   const [poolsSubTab, setPoolsSubTab] = useState("All");
...
//       let poolTypeFilter = "!clmm"; // Default for "rwa"
//       if (poolsSubTab === "clmm") {
//         poolTypeFilter = "clmm";
//       } else if (poolsSubTab === "rwa") {
//         poolTypeFilter = "!clmm"; // RWA shows !clmm
//       }
//       // For "All", we'll fetch both CLMM and RWA pools separately

//       // If "All" is selected, fetch both CLMM and RWA pools
//       if (poolsSubTab === "All") {
//         console.log("===========================================");
//         console.log("📊 FETCHING ALL POOLS DATA (CLMM + RWA)");
//         console.log("===========================================");

//         // Fetch CLMM pools
//         const clmmResponse = await ADMIN_API.GET_LIQUIDITY_POOLS({
//           tokenAddress,
//           page,
//           limit,
//           poolType: "clmm",
...
//         // Fetch RWA pools
//         const rwaResponse = await ADMIN_API.GET_LIQUIDITY_POOLS({
//           tokenAddress,
//           page,
//           limit,
//           poolType: "!clmm",
...
//         let allPools = [];
//         let poolsTotalCount = 0;

//         if (clmmResponse && clmmResponse.status === 200) {
//           const clmmPools = clmmResponse?.data?.data || [];
//           allPools = [...allPools, ...clmmPools];
//           poolsTotalCount += clmmResponse?.data?.total ?? 0;
//           console.log("✅ CLMM pools fetched:", clmmPools.length);
//         }

//         if (rwaResponse && rwaResponse.status === 200) {
//           const rwaPools = rwaResponse?.data?.data || [];
//           allPools = [...allPools, ...rwaPools];
//           poolsTotalCount += rwaResponse?.data?.total ?? 0;
//           console.log("✅ RWA pools fetched:", rwaPools.length);
//         }

//         console.log("📦 Total pools (CLMM + RWA):", allPools.length);
//         console.log("===========================================");

//         // Transform the API data to match our table structure
//         const transformedPools = allPools.map((pool, index) => ({
//           id: pool._id,
//           name: pool.name,
//           symbol: pool.symbol,
...
//           poolAddress: pool.pairAddress || pool.tokenAddress,
...
//         const apiTotalPages = poolsTotalCount > 0 ? Math.ceil(poolsTotalCount / apiLimit) : 1;
//         setPoolsTotalPages(apiTotalPages);
//         setPoolsTotalCount(poolsTotalCount);
...
//         }
//       } else {
//         // For "clmm" or "rwa" filters, use single API call
...
//   // Fetch live pool data from GET_LIVE_POOLS_DATA or GET_LIVE_POOLS_NOT_RWA_DATA API
...
//       // Find the selected token to get chainId
//       const selectedToken = pooledTokensData.find(
//         (t) => t.tokenAddress === tokenAddress
//       );
//       const chainId = selectedToken?.chainId || "solana";

//       // Check if we're fetching CLMM pools or All pools (which includes CLMM)
//       const isCLMM = poolsSubTab === "clmm";
//       const isAll = poolsSubTab === "All";

//       // Separate CLMM and RWA pools
//       const clmmPools = pools.filter(p => p.poolType === "clmm");
//       const rwaPools = pools.filter(p => p.poolType !== "clmm");

//       // Combined live data map for all pools
//       const combinedLiveDataMap = {};

//       // Fetch CLMM pools if needed
//       if ((isCLMM || isAll) && clmmPools.length > 0) {
//         // Use GET_LIVE_POOLS_NOT_RWA_DATA for CLMM pools (fetch all at once)
//         console.log("===========================================");
//         console.log("📈 FETCHING CLMM LIVE POOL DATA (BATCH)");
//         console.log("===========================================");
//         console.log("Number of CLMM pools to fetch:", clmmPools.length);
//         console.log("API Endpoint: /api/v1/main/livePoolsNotRWA");

//         // Construct full URL for CLMM live pool data
//         const baseURL = import.meta.env.VITE_API_URL;
//         const clmmEndpoint = "/api/v1/main/livePoolsNotRWA";
//         const clmmQueryParams = new URLSearchParams({
//           tokenAddress: tokenAddress,
//           chainId: chainId,
//           page: "1",
//           limit: clmmPools.length.toString(),
//         });
//         const clmmFullUrl = `${baseURL}${clmmEndpoint}?${clmmQueryParams.toString()}`;
        
//         console.log("Full URL:", clmmFullUrl);
//         console.log("Parameters:", {
//           tokenAddress: tokenAddress,
//           chainId: chainId,
//           page: 1,
//           limit: clmmPools.length,
//         });
...
//           if (response && response.status === 200) {
//             console.log("✅ CLMM API Response Status:", response.status);
//             console.log("📦 CLMM API Response Data:", JSON.stringify(response.data, null, 2));
            
//             const responsePools = response?.data?.pools || response?.data?.data || [];

//             console.log("Response pools count:", responsePools.length);
//             console.log("Our CLMM pools count:", clmmPools.length);

//             // Map response pools to our pool structure
//             responsePools.forEach((livePool) => {
//               const fullPoolData = livePool.fullPoolData || livePool;
//               const pairAddress = livePool.pairAddress || fullPoolData?.pairAddress;
              
//               if (!pairAddress) return;
              
//               // Find matching pool from our list by pairAddress
//               const matchingPool = clmmPools.find(p => 
//                 p.pairAddress === pairAddress
//               );

//               if (matchingPool && fullPoolData) {
//                 console.log(`   ✅ Matched CLMM pool: ${matchingPool.name || matchingPool.symbol} (${pairAddress})`);
//                 combinedLiveDataMap[matchingPool.id] = {
...
//                 };
//               } else {
//                 console.log(`   ⚠️ No matching CLMM pool found for pairAddress: ${pairAddress}`);
//               }
//             });

//             console.log("✅ CLMM live pool data fetched:", Object.keys(combinedLiveDataMap).length, "pools");
//           }
//         } catch (error) {
//           console.error("Error fetching CLMM live pool data:", error);
//         }
//       }

//       // Fetch RWA pools (either for "All" filter or "rwa" filter)
//       const poolsToFetch = isAll ? rwaPools : (isCLMM ? [] : pools);
//       if (poolsToFetch.length > 0) {
        
//         // Use GET_LIVE_POOLS_DATA for RWA pools (fetch individually)
//         console.log("===========================================");
//         console.log("📈 FETCHING RWA LIVE POOL DATA");
//         console.log("===========================================");
//         console.log("Number of RWA pools to fetch:", poolsToFetch.length);
//         console.log("API Endpoint: /api/v1/main/livePoolsData");
        
//         const liveDataPromises = poolsToFetch.map(async (pool) => {
//           try {
//             // Construct full URL for live pool data
//             const baseURL = import.meta.env.VITE_API_URL;
//             const liveEndpoint = "/api/v1/main/livePoolsData";
...
//             const response = await ADMIN_API.GET_LIVE_POOLS_DATA({
//               tokenAddress: pool.tokenAddress,
//               pairAddress: pool.pairAddress,
//               chainId: chainId,
//             });
...
//               console.log(`   ✅ Success! Live data received for ${pool.name || pool.symbol}`);
...
//             combinedLiveDataMap[result.poolId] = result.data;
//           }
//         });

//         console.log("✅ RWA live pool data fetched:", Object.keys(combinedLiveDataMap).length - (isAll ? clmmPools.length : 0), "pools");
//       }

//       // Update state with combined data (CLMM + RWA)
//       console.log("\n✅ All live pool data fetched successfully!");
//       console.log("Total pools with live data:", Object.keys(combinedLiveDataMap).length);
//       console.log("===========================================\n");

//       setLivePoolData(combinedLiveDataMap);

//       // Update liquidity pools with live data (merge both CLMM and RWA)
//       setLiquidityPools((prevPools) =>
//         prevPools.map((pool) => {
//           const liveData = combinedLiveDataMap[pool.id];
//           if (liveData) {
//             return {
//               ...pool,
...
//     // Filter based on poolsSubTab
//     if (poolsSubTab === "All") {
//       // All shows both CLMM and RWA pools
//       return true;
//     } else if (poolsSubTab === "clmm") {
...
//   // Reset to page 1 when token, reportsSubTab, or poolsSubTab changes
//   useEffect(() => {
//     if (selectedPoolToken) {
//       setCurrentPage(1);
//     }
...
//       let poolType = "clmm+rwa"; // Default for "All" tab
//       if (reportsSubTab === "clmm") {
//         poolType = "clmm";
//       } else if (reportsSubTab === "rwa") {
//         poolType = "rwa";
//       }
...
//       console.log("   Active Tab:", reportsSubTab);
//       console.log("   Selected PoolType Filter:", poolType);
//       console.log("   Token Address:", tokenAddress);
//       console.log("   Token Name:", selectedToken?.name || "N/A");
//       console.log("===========================================");
...
//         console.log("   Current Page:", response.data?.currentPage || page);
//         console.log("   Total Pages:", apiTotalPages);
//         console.log("   Limit per Page:", response.data?.limit || rowsPerPage);
//         console.log("===========================================");
        
//         // Log poolType breakdown in received data
//         if (reports.length > 0) {
//           const poolTypeCounts = {};
//           reports.forEach((report) => {
//             const pt = report.poolType || "unknown";
//             poolTypeCounts[pt] = (poolTypeCounts[pt] || 0) + 1;
//           });
          
//           console.log("🏷️  POOL TYPE BREAKDOWN IN RECEIVED DATA:");
//           Object.entries(poolTypeCounts).forEach(([pt, count]) => {
//             console.log(`   ${pt}: ${count} report(s)`);
//           });
//           console.log("===========================================");
          
//           // Log sample data (first 2 reports)
//           console.log("📋 SAMPLE DATA (First 2 Reports):");
//           reports.slice(0, 2).forEach((report, index) => {
//             console.log(`   Report ${index + 1}:`, {
//               endTime: report.endTime,
//               poolType: report.poolType,
...
//           console.log("   Filter Applied:", poolType);
//           console.log("   Active Tab:", reportsSubTab);
//           console.log("===========================================");
//         }
...
//         console.log("   Response:", response?.data || "N/A");
//         console.log("===========================================\n");
//       }
//     } catch (error) {
...
//   // Fetch liquidity pools when token/page/limit/poolsSubTab changes and Pools tab is active
//   useEffect(() => {
//     if (selectedPoolToken && activeTab === "Pools") {
//       fetchLiquidityPools(selectedPoolToken, currentPage, rowsPerPage);
//     }
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [selectedPoolToken, currentPage, rowsPerPage, activeTab, poolsSubTab]);

//   // Fetch reports when token/page/limit/reportsSubTab changes and Reports tab is active
//   useEffect(() => {
...
//       // Always use /rwa route for RWA Trading Pools page
//       const route = "/rwa";

//       // Use the chainId from the selected token and addresses from the pool
//       navigate(
//         `${route}/${selectedToken.chainId}/${pool.pairAddress}/${pool.tokenAddress}`
//       );
//     } else {
...
//                 Real World Asset pools across multiple platforms
//               </p>
//             </div>
...
const RWATable = () => {
  ...
  const [poolsRowsPerPage, setPoolsRowsPerPage] = useState(20);
  ...
  const [liquidityPools, setLiquidityPools] = useState([]);
  const [livePoolData, setLivePoolData] = useState({});
  const [activeTab, setActiveTab] = useState(user?.role === "superuser" ? "Pools" : "Reports");
  const [reportsSubTab, setReportsSubTab] = useState("All");
  const [poolsSubTab, setPoolsSubTab] = useState("All");
  const [reportsData, setReportsData] = useState([]);
  const [reportsLoading, setReportsLoading] = useState(false);
  ...
  const fetchLiquidityPools = async (
    ...
    try {
      ...
      if (poolsSubTab === "All") {
        ...
        const apiLimit = poolsRowsPerPage;
        const apiTotalPages = poolsTotalCount > 0 ? Math.ceil(poolsTotalCount / apiLimit) : 1;
        setPoolsTotalPages(apiTotalPages);
        setPoolsTotalCount(poolsTotalCount);
        setLiquidityPools(transformedPools);
  ...
  const refreshData = useCallback(() => {
    if (selectedPoolToken) {
      if (activeTab === "Pools") {
        fetchLiquidityPools(selectedPoolToken, poolsCurrentPage, poolsRowsPerPage);
      } else if (activeTab === "Reports") {
        fetchDailyPoolReportsAggregates(selectedPoolToken, reportsCurrentPage, reportsRowsPerPage);
      }
    }
  }, [selectedPoolToken, poolsCurrentPage, poolsRowsPerPage, reportsCurrentPage, reportsRowsPerPage, activeTab]);

  // Handle page change
  const handlePoolsPageChange = (newPage) => {
    if (newPage < 1 || newPage > poolsTotalPages || newPage === poolsCurrentPage) {
      return;
    }
...
console.log("liquidityPools" ,liquidityPools);
  ...
  const fetchDailyPoolReportsAggregates = async (
    ...
    try {
      ...
      if (response && response.status === 200) {
        ...
        const reports = response?.data?.data || [];
        const apiTotal = response?.data?.totalCount || reports.length;
        const apiTotalPages = response?.data?.totalPages || 1;
        const apiLimit = response?.data?.limit || reportsRowsPerPage;
        ...
        console.log("   Current Page:", response.data?.currentPage || page);
        console.log("   Total Pages:", apiTotalPages);
        console.log("   Limit per Page:", apiLimit);
        console.log("===========================================");
        ...
        if (response?.data?.limit && response.data.limit !== reportsRowsPerPage) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedPoolToken, poolsCurrentPage, poolsRowsPerPage, activeTab, poolsSubTab]);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedPoolToken, reportsCurrentPage, reportsRowsPerPage, activeTab, reportsSubTab]);

  // Sort function
  const sortedPools = useMemo(() => {
    const sortablePools = [...currentPools];
    console.log(" currentPools before sort:", currentPools);
  ...
  const reportsTableColumns = [
    ...
    { key: "totalTransactions", label: "Transactions", width: "w-auto", sortable: true },
    { key: "buys", label: "Buys", width: "w-auto", sortable: true },
    { key: "sells", label: "Sells", width: "w-auto", sortable: true },
    { key: "totalVolume", label: "Volume", width: "w-auto", sortable: true },
    { key: "lpAdd", label: "Pool Fee", width: "w-auto", sortable: true },
    { key: "solAverage", label: "Sol Avg", width: "w-auto", sortable: true },
    { key: "gasFee", label: "Gas Fee", width: "w-auto", sortable: true },
    { key: "gasFeeInDollars", label: "Gas fee ( $ )", width: "w-auto", sortable: true },
                    ...
                    <div className="px-8 py-6 border-t border-white/20 bg-gradient-to-r from-purple-50/50 to-blue-50/50">
                      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                        <p className="text-sm text-gray-600">
                          ...
                          to{" "}
                          <span className="font-semibold text-gray-900">
                            {Math.min(reportsCurrentPage * reportsRowsPerPage, reportsTotalCount)}
                          </span>{" "}
                          of{" "}
                          <span className="font-semibold text-gray-900">
                            {reportsTotalCount}
                  ...
                  <div className="px-8 py-6 border-t border-white/20 bg-gradient-to-r from-purple-50/50 to-blue-50/50">
                    {paginatedPools.length > 0 && (
                      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                        ...
                        <div className="flex gap-2">
                          <button
                            onClick={() => handlePoolsPageChange(poolsCurrentPage - 1)}
                          ...
                          <div className="flex items-center gap-1">
                            {Array.from({ length: Math.min(5, poolsTotalPages) }, (_, i) => {
                              ...
                              if (poolsTotalPages <= 5) {
                                ...
                                pageNum = i + 1;
                              } else if (poolsCurrentPage >= poolsTotalPages - 2) {
                                pageNum = poolsTotalPages - 4 + i;
                              } else {
                              ...
                              return (
                                <button
                                  key={pageNum}
                                  onClick={() => handlePoolsPageChange(pageNum)}
                                  className={`px-2 py-1.5 text-xs font-medium rounded-lg transition-colors ${
