...
export default function LPAddedDonutChart({ tokenVolumeData }) {
  ...
  // Process dynamic data for LP Added
  const processTokenData = () => {
    ...
    // Sanitize data to ensure totalPoolRevenue is a number
    const sanitizedData = tokenVolumeData.map((t) => ({
      ...t,
      totalPoolRevenue: Number(t.totalPoolRevenue) || 0,
    }));

    const sortedData = [...sanitizedData].sort(
      (a, b) => b.totalPoolRevenue - a.totalPoolRevenue
    );

    const visibleTokens = sortedData.slice(0, 4); // Show top 4 for better UI
    const others = sortedData.slice(4);
    const othersTotal = others.reduce((sum, t) => sum + t.totalPoolRevenue, 0);
    const totalLpAdded = sortedData.reduce((sum, t) => sum + t.totalPoolRevenue, 0);
  ...
  const { sortedData, visibleTokens, others, othersTotal, totalLpAdded } =
    processTokenData();
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const percentage =
        totalLpAdded > 0 ? (data.value / totalLpAdded) * 100 : 0;
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4">
        ...
        <div className="flex-1 space-y-2 min-w-0">
          {chartData.map((item, index) => {
            const percentage =
              totalLpAdded > 0 ? (item.value / totalLpAdded) * 100 : 0;
            return (
