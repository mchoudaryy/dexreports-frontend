...
export default function LPAddedChart({
  chartData,
  loading,
  selectedToken,
  timeFrame,
  ...
  const totalLpAdd = useMemo(
    () => chartData.reduce((sum, item) => sum + item.lpAdd, 0),
    [chartData]
  );

  const maxValue = useMemo(() => {
    if (chartData.length === 0) return 1000;
    const maxLpAdd = Math.max(...chartData.map((item) => item.lpAdd));
    return Math.ceil(maxLpAdd / 1000) * 1000;
  }, [chartData]);
    ...
    <Card className="w-full p-4 h-full">
      ...
      <div className="flex items-center gap-3 mb-4">
        {/* <div
          ...
          style={{ backgroundColor: colors.lpAdd.primary }}
        />
        ...
        <span className="text-sm font-bold text-gray-900">
          $
          {totalLpAdd.toLocaleString(undefined, {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })}
        </span> */}
            ...
            >
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <Bar
                    dataKey="lpAdd"
                    fill={colors.lpAdd.primary}
                    radius={[4, 4, 0, 0]}
                    maxBarSize={50}
                    onMouseEnter={() => setHoveredBar("lpAdd")}
                  ...
                  >
                    {chartData.map((entry, index) => (
                      <Cell
                        key={`cell-lpAdd-${index}`}
                        fill={
                          hoveredBar === "lpAdd"
                            ? colors.lpAdd.hover
                            : colors.lpAdd.primary
                        }
                        opacity={
                          hoveredBar === null || hoveredBar === "lpAdd"
                            ? 1
                            : 0.5
                        }
                        style={{
